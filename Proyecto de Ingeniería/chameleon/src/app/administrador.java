...
public class administrador extends gerente {
	...
	public void agregarNuevoUsuario(Scanner sin) {
		...
		String r[] = new String[5]; // Cada dato está separado por (;) y esos datos se guardan en el array
		boolean check = false;
		int i;
		do {
			if (check)
				System.out.println("Los datos introducidos no son válidos.\nNo se aceptan datos vacíos ni datos con '_'");
	...
	public void eliminarUsuario(String idUsuario) {
		...
		if (listaUsuarios.size() != 1) {
			...
			if (i != listaUsuarios.size()) {
				if (!this.getId().equals(listaUsuarios.get(i).getId())) { // Compara con el ID de este objeto administrador
			...
			System.out.println("No es posible eliminar el único usuario en el sistema");
